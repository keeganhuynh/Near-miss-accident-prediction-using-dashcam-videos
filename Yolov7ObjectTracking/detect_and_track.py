...
def draw_boxes(img, bbox, risk, identities=None, categories=None, names=None, save_with_object_id=False, path=None,offset=(0, 0)):
...
def detect(file_source, vnp, speed, json_file_path,save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                ego_car = uv_to_world((img_shape[1]//2,img_shape[0]) , 1.31, vnp[idx], img_shape, FOV)
                ...
                for track in tracks:
                    ...
                    # print(idx, int(track.id+1))
                    if (int(track.detclass) < 10 and X <= 4 and Z <= 5):
                      c = class_name[int(track.detclass)]

                      x1, y1 = img_shape[1]//2, img_shape[0]
                      x2, y2 = int(x), int(y)
                      place = (int(0.5*(x1+x2)), int(0.5*(y1+y2)))
                      color = (255, 0, 0)  # Màu xanh lá cây
                      font = cv2.FONT_HERSHEY_SIMPLEX
                      font_scale = 1
                      thickness = 1
                      text = f'{round(X,1)},{round(Z,1)}'

                      cv2.putText(im0, text, place, font, font_scale, color, thickness)
                      # cv2.line(im0, (x1, y1), (x2, y2), color, thickness=2) 
                    point_color = (0, 0, 255)
                    point_size = 5
                ...
                risk = PredictRisk(idx, obj_list, traj_step, predict_step, object_track, speed[idx])
...
def process(video_path, vnp_path, veclocity_path, json_save_path, fps):
    '''
    ...
    with torch.no_grad():
        detect(video_path, vnp, speed, json_save_path)    

def TrajectoryAndMakingVideo(video_path, vnp_path, veclocity_path, json_file_path, fps):
