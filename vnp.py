...
def FirstStage(folderpath, output_path, videopath, stage, frame_interval, fallback_RVP=None):
    fps = 0
    frame_count = 0
    if (stage == 0):
    ...
    if (stage == 3):
        ...
        fps, frame_count = ProcessVideo(folderpath, videopath, frame_interval)
        print('===============Run deep hough=================')
        VanishingPointDetection(output_path, videopath, frame_interval, fallback_RVP=fallback_RVP)

    return fps, frame_count


def MakeInput(folderpath, output_path, videopath, frame_interval=3, stage=2, fallback_RVP=None):
    if fallback_RVP:
        return FirstStage(folderpath, output_path, videopath, stage, frame_interval, fallback_RVP)
    else:
